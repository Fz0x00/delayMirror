...
async fn proxy_upstream_streaming(url: &str, client: &Client) -> Response {
    match client.get(url).send().await {
        Ok(resp) => {
            ...
            builder
                ...
                .unwrap_or_else(|_| error_response(500, "stream error"))
        }
        Err(e) => error_response(502, &format!("upstream error: {}", e)),
    }
}

async fn proxy_upstream_streaming_with_headers(
    url: &str,
    client: &Client,
    headers: Vec<(&'static str, String)>,
) -> Response {
    let mut resp = proxy_upstream_streaming(url, client).await;
    for (name, value) in headers {
...
fn error_response_with_headers(
    ...
    error_response_json_with_headers(status, body, headers)
}
